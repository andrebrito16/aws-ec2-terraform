# Assuming 8 public and 8 private subnets
...
resource "aws_subnet" "private_subnets" {
  ...
  vpc_id                                      = aws_vpc.main_vpc.id
  availability_zone_id                        = data.aws_availability_zones.available_az.zone_ids[count.index]
  cidr_block                                  = cidrsubnet(var.vpc_cidr_block, 4, count.index + length(data.aws_availability_zones.available_az.zone_ids))
